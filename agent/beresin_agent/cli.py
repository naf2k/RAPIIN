"""BERESIN Desktop Agent CLI.

Commands:
  beresin-agent setup [--server URL]   first-run: login + register device
  beresin-agent run                    poll loop (foreground)
  beresin-agent status                 show connection/device status
  beresin-agent autostart [on|off]     manage OS autostart
  beresin-agent logout                 remove local credentials
...
def cmd_setup(args) -> int:
    print("=== Setup BERESIN Desktop Agent ===")
...
def cmd_run(args) -> int:
    if not is_configured():
        print("Agent belum disetup. Jalankan 'beresin setup' dulu.", file=sys.stderr)
        return 1
    cfg = load_config()
    api = AgentAPI(cfg["server_url"])
    print(f"BERESIN Agent {__version__} berjalan untuk {cfg['email']} (device {cfg['device_id']})")
...
def cmd_verify(args) -> int:
    ...
    if not is_configured():
        print("[gagal] Agent belum disetup. Jalankan 'beresin setup' dulu.", file=sys.stderr)
        return 1
    cfg = load_config()
    api = AgentAPI(cfg["server_url"])
    try:
...
def _autostart_paths():
    ...
    if sys.platform == "darwin":
        return [Path.home() / "Library" / "LaunchAgents" / "com.beresin.agent.plist"]
    if sys.platform.startswith("linux"):
        return [Path.home() / ".config" / "systemd" / "user" / "beresin-agent.service"]
    if sys.platform == "win32":
        return [Path.home() / "AppData" / "Roaming" / "Microsoft" / "Windows" / "Start Menu" / "Programs" / "Startup" / "beresin-agent.cmd"]
    return []
...
def _agent_run_command() -> list[str]:
    ...
    return [sys.executable, "-m", "beresin_agent.cli", "run"]
...
def _systemd_service():
    ...
    svc = service_dir / "beresin-agent.service"
    command = " ".join(shlex.quote(part) for part in _agent_run_command())
    svc.write_text(
        f"""[Unit]
Description=BERESIN Desktop Agent
...
def _enable_autostart() -> None:
    try:
        if sys.platform == "darwin":
            ...
            subprocess.run(["systemctl", "--user", "enable", "--now", "beresin-agent.service"], check=False)
            ...
            (startup / "beresin-agent.cmd").write_text(f"@echo off\n{command}\n")
...
def _disable_autostart() -> None:
    """Unload/disable the service and remove only BERESIN's exact startup file."""
    import subprocess
    ...
    try:
        if sys.platform == "darwin" and targets:
            ...
            subprocess.run(["systemctl", "--user", "disable", "--now", "beresin-agent.service"], check=False)
...
def cmd_upgrade(args) -> int:
    ...
    if not wheel.is_file() or wheel.suffix != ".whl" or "beresin_agent-" not in wheel.name:
        print("[gagal] Gunakan wheel BERESIN Agent yang valid.", file=sys.stderr)
        return 2
    uv = shutil.which("uv")
    if uv:
    ...
    print(f"[ok] BERESIN Agent diperbarui dari {wheel.name}")
    return 0
...
def cmd_uninstall(args) -> int:
    ...
    if not args.keep_config:
        clear_config()
        print("[ok] Kredensial dan state lokal BERESIN dihapus")
    if shutil.which("uv"):
        print("Hapus paket dengan: uv tool uninstall beresin-agent")
    else:
        print(f"Hapus paket dengan: {sys.executable} -m pip uninstall beresin-agent")
    return 0
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="beresin", description="BERESIN Desktop Agent")
    ...
    p_upgrade.add_argument("--wheel", required=True, help="path wheel beresin_agent")
