...
- npm and Python dependency audits: zero known vulnerabilities (the unpublished local `rapiin-agent` package itself is not present on PyPI and is therefore skipped by `pip-audit`).
- Terminal wheel exposes both the PRD `rapiin` command and the administrative `rapiin-agent` alias.
- SQLite WAL backup/restore drill: identical SHA-256 backup and restored database, integrity check `ok`; empty/non-RAPIIN databases are rejected.
...
- Local macOS reboot drill: `com.rapiin.server` and `com.rapiin.agent` started automatically after login; `/ready`, SQLite integrity, device verification, queue readiness, health monitoring, and credentialed AI UAT all passed.
...
- Readiness CLI (`rapiin-ops verify`): all checks green — server, PostgreSQL, roles, audit hash chain, provider circuit closed, assignment queue empty, Redis, pinned Hermes version, Operations enabled, Telegram configured, LaunchAgents loaded, fresh backups.
...
- **Ops CLI entry point:** `python -m rapiin.ops_cli verify` no longer exits silently without a report. Regression test added.
- **Redis test isolation:** with `RAPIIN_TEST_DATABASE_URL` set, tests now use a dedicated Redis logical database (15). Previously the shared queue key let the live conversation worker consume test jobs and could apply test task ids against the pilot database.
...
- Drill incidents #5, #6, and #9 were closed, and their drill-only code changes, code approvals, and check runs were removed after the uncommitted drill patch was archived outside the repository (`~/RapiinBackups/drill-artifacts/`). The audit log chain remains valid and intact.
...
- Incident #2 (`Disk space critically low`) remains correctly OPEN: the host volume is genuinely below the 10% free threshold used by `ops/local_health_monitor.py`. Resolving it requires the owner to free space on the Mac; RAPIIN's own footprint is about 15 MB of data plus backups.

The pilot database now contains exactly two active supervisor accounts as required by PRD section 3. Two obsolete local/test identities were removed after creating the recoverable SQLite backup `server/data/backups/rapiin-20260908T115222Z.db`.
