...
class Settings(BaseSettings):
    ...
    # Server
    rapiin_db_path: str = "./data/rapiin.db"
    rapiin_database_url: str = ""
    rapiin_redis_url: str = ""
    rapiin_embedded_queue_worker: bool = False
    rapiin_data_dir: str = "./data"
    rapiin_secret_key: str = "dev-secret-change-me"
    rapiin_host: str = "127.0.0.1"
    rapiin_port: int = 8000
    rapiin_env: str = "development"
    rapiin_allowed_origins: str = "http://localhost:8000,http://127.0.0.1:8000"
    rapiin_secret_key_file: str = ""
    rapiin_init_supervisor_password_file: str = ""
    rapiin_monitoring_token: str = ""
    rapiin_monitoring_token_file: str = ""
    rapiin_allow_public_registration: bool = True
    ...
    ops_github_repo: str = "naf2k/RAPIIN"
    ...
    # Seeded accounts
    rapiin_init_supervisor_email: str = "supervisor@rapiin.example.com"
    rapiin_init_supervisor_password: str = "Supervisor123!"

    @property
    def db_path(self) -> Path:
        path = Path(self.rapiin_db_path)
    ...
    def data_dir(self) -> Path:
        path = Path(self.rapiin_data_dir)
        if not path.is_absolute():
            path = BASE_DIR / path
        return path
    ...
    def allowed_origins(self) -> list[str]:
        return [origin.strip() for origin in self.rapiin_allowed_origins.split(",") if origin.strip()]

    def validate_for_startup(self) -> None:
        for value_field, file_field in (
            ("rapiin_secret_key", "rapiin_secret_key_file"),
            ("rapiin_init_supervisor_password", "rapiin_init_supervisor_password_file"),
            ("rapiin_monitoring_token", "rapiin_monitoring_token_file"),
        ...
        if self.rapiin_env.lower() != "production":
            return
        errors = []
        if self.rapiin_secret_key in {"dev-secret-change-me", "change-me-in-production", ""} or len(self.rapiin_secret_key) < 32:
            errors.append("RAPIIN_SECRET_KEY wajib unik dan minimal 32 karakter")
        if self.rapiin_init_supervisor_email == "supervisor@rapiin.example.com":
            errors.append("email supervisor production wajib diganti")
        if self.rapiin_init_supervisor_password == "Supervisor123!" or len(self.rapiin_init_supervisor_password) < 12:
        ...
        if len(self.rapiin_monitoring_token) < 32:
            errors.append("RAPIIN_MONITORING_TOKEN wajib minimal 32 karakter")
        if self.rapiin_allow_public_registration:
            errors.append("RAPIIN_ALLOW_PUBLIC_REGISTRATION wajib false di production")
        ...
        if bool(self.rapiin_database_url) != bool(self.rapiin_redis_url):
            errors.append("RAPIIN_DATABASE_URL dan RAPIIN_REDIS_URL wajib diaktifkan bersama di production")
        if self.rapiin_redis_url and self.rapiin_embedded_queue_worker:
            errors.append("RAPIIN_EMBEDDED_QUEUE_WORKER wajib false saat Redis digunakan di production")
        if errors:
