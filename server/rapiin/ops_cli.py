...
def readiness() -> dict:
    ...
    try:
        ...
        record("database", True, "postgresql" if settings.rapiin_database_url else "sqlite")
    ...
    record("redis", redis_ready(), "configured" if settings.rapiin_redis_url else "inline")
    ...
    if shutil.which("launchctl"):
        ...
        labels = {"com.rapiin.server", "com.rapiin.agent", "com.rapiin.monitor", "com.rapiin.backup"}
        if settings.rapiin_redis_url:
            labels.add("com.rapiin.worker")
    ...
    backups = list((Path.home() / "RapiinBackups").glob("*"))
...
def main() -> int:
    parser = argparse.ArgumentParser(prog="rapiin-ops")
