...
def _sandbox_profile(worktree: Path, hermes_command: str) -> str:
    ...
    protected_reads = [
        ...
        home / ".rapiin", REPO_ROOT / "server" / ".env", REPO_ROOT / "server" / "data" / "secrets",
    ]
...
def run_coder(conn, code_change_id: int, runner=None) -> dict:
    ...
    prompt = (
        "Perbaiki insiden RAPIIN berikut hanya di worktree saat ini. Jangan deploy, jangan mengakses dokumen pengguna, "
...
def run_checks(conn, code_change_id: int) -> list[dict]:
    ...
    if bandit.is_file():
        commands.append(("security-bandit", [str(bandit), "-q", "-r", "server/rapiin", "-lll", "-ii"]))
    conn.execute(
...
def create_pull_request(conn, code_change_id: int) -> dict:
    ...
    pr = _run(["gh", "pr", "create", "--repo", settings.ops_github_repo, "--base", "main", "--head", change["branch_name"], "--title", f"Fix Operations incident #{change['incident_id']}", "--body", f"Generated from approved RAPIIN incident #{change['incident_id']}. Deployment still requires separate approval."], worktree, timeout=120)
    if pr.returncode:
