...
from rapiin.queue_backend import dequeue_conversation_task, enqueue_conversation_task, redis_ready
...
def test_redis_queue_round_trip(monkeypatch):
    fake = FakeRedis()
    monkeypatch.setattr("rapiin.queue_backend.redis_client", lambda: fake)
...
def test_blocking_read_outlasts_brpop_timeout(monkeypatch):
    ...
    """
    from rapiin.queue_backend import SOCKET_TIMEOUT_SECONDS, redis_client
    ...
    monkeypatch.setattr("rapiin.config.settings.rapiin_redis_url", "redis://127.0.0.1:6379/0")
...
def test_transient_redis_failure_is_not_fatal(monkeypatch):
    monkeypatch.setattr("rapiin.queue_backend.redis_client", lambda: FailingRedis())
...
def test_queue_worker_survives_transient_redis_failure(monkeypatch):
    ...
    monkeypatch.setattr("rapiin.queue_backend.dequeue_conversation_task", failing_then_stopping)
