...
(function () {
  ...
  function renderOpsApprovals(items) {
    ...
    pending.forEach(function (item) { var card = document.createElement("article"); card.className = "approval-item"; var copy = document.createElement("span"); copy.className = "item-copy"; copy.innerHTML = "<strong>" + BERESIN.esc(item.approval_type) + "</strong><span>Incident #" + item.incident_id + "</span><small>Approval #" + item.id + "</small>"; var actions = document.createElement("span"); actions.className = "ops-actions"; ["APPROVED", "REJECTED"].forEach(function (decision) { var b = document.createElement("button"); b.className = decision === "APPROVED" ? "button button--primary" : "button button--secondary"; b.textContent = decision === "APPROVED" ? "Approve" : "Reject"; b.addEventListener("click", async function () { var password = null; if (decision === "APPROVED") { password = window.prompt("Masukkan ulang password supervisor untuk menyetujui tindakan ini:"); if (!password) return; } await BERESIN.api("POST", "/supervisor/ops/approvals/" + item.id + "/respond", {decision: decision, note: "Diputuskan oleh owner", password: password}); BERESIN.showToast("Keputusan disimpan.", "success"); await loadOperations(false); }); actions.appendChild(b); }); card.appendChild(copy); card.appendChild(actions); box.appendChild(card); });
  }
  ...
  function renderOpsAgents(items, usage) {
    ...
    items.forEach(function (item) { var policy = {}; try { policy = JSON.parse(item.tool_policy || "{}"); } catch (_) {} var stat = stats[item.role] || {runs:0,succeeded:0,failed:0,duration_ms:0,input_tokens:0,output_tokens:0,api_calls:0,estimated_cost_usd:0}; var card = document.createElement("article"); card.className = "ops-agent-card"; card.innerHTML = "<span>" + BERESIN.esc(item.state) + "</span><h3>" + BERESIN.esc(item.role) + "</h3><p>" + (policy.read_only ? "Read-only" : "Perubahan hanya setelah approval") + (policy.isolated_worktree ? " · isolated worktree" : "") + "</p><small>Hari ini: " + Number(stat.runs) + " run · " + Number(stat.succeeded) + " berhasil · " + Number(stat.failed) + " gagal · " + Math.round(Number(stat.duration_ms) / 1000) + " dtk<br>" + Number(stat.api_calls) + " API call · " + (Number(stat.input_tokens) + Number(stat.output_tokens)).toLocaleString("id-ID") + " token · estimasi $" + Number(stat.estimated_cost_usd).toFixed(4) + "</small>"; box.appendChild(card); });
  }
